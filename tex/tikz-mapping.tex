...
\begin{tikzpicture}[
    ...
    \node (qi-label) at (qi.east) [right] {$\StateOutputEvent{q_i\!} = o_1$};
    \node (qj-label) at (qj.east) [right] {$\StateOutputEvent{q_j\!} = o_3$};
    ...
    \path[->]
        (qi) edge[right] node {
            \(\begin{aligned}
                & \ActualTransitionFunction{q_i,e_1,u_1\!} = q_0 \\[\LineGapMore]
                & \ActualTransitionFunction{q_i,e_2,u_2\!} = q_j
            \end{aligned}\)} (qj)
