...
Наивный подход заключался бы в том, чтобы запретить каждый отрицательный сценарий для всех модулей, но это было бы некорректно: поведение, которое является неправильным для одного из модулей, может быть совершенно нормальным для другого.
...
Ключевая идея для преодоления этой проблемы заключается в том, чтобы убедиться, что каждый отрицательный сценарий запрещён хотя бы для одного модуля.
...
\subsection{Сведение к SAT для распределённого синтеза из положительных сценариев}
\label{sec:distributed:modular}
...
Все остальные части кодировки из раздела~\ref{sec:monolith-synthesis} расширяются аналогичным образом, за исключением отрицательного отображения, которое мы обсудим ниже.

Различие с положительными сценариями заключается в том, что теперь у нас есть набор деревьев положительных сценариев $\Set{\ModularPositiveTree}_{m \in \Modules}$, по одному для каждого модуля распределённого контроллера.
Переменная~$\ModularMapping_{v}$ обозначает состояние, в~котором автомат для $m$-го модуля завершает обработку последовательности входных данных, образованной следованием по положительному дереву от корня до узла~$v$ (поскольку дерево детерминированное, для каждого узла~$v$ существует только один путь от корня).
